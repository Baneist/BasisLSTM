...
class Basisformer(nn.Module):
    def __init__(self, seq_len, pred_len, d_model, heads, basis_nums, block_nums, bottle, map_bottleneck, device, tau, v_num, d_layer):
        super().__init__()
        ...
        self.p_seq_len_x = self.seq_len // self.patch
        self.p_seq_len_y = self.pred_len // self.patch
        ...
        self.mlstmnet = mLSTM(v_num, self.N, self.k, d_model, d_layer, self.patch, device=device)
        self.device = device
    ...
    def forward(self, x, mark, y=None, train=True, y_mark=None):
        ...
        m1 = raw_m1.permute(0,2,1).view(B,self.N,self.p_seq_len_x,-1).permute(0,2,1,3)
        feature = feature.permute(0,2,1).view(B,C,self.p_seq_len_x,-1).permute(0,2,1,3)
        score = self.mlstmnet(feature,m1)
        ...
        out = self.MLP_sy(out).reshape(B, C, -1).permute(0, 2, 1)  # （B,C,L）
        ...
        if train:
            ...
            m2 = m[:, self.seq_len:].permute(0, 2, 1).view(B, self.N, self.p_seq_len_y, -1).permute(0,2,1,3)
            feature_y = feature_y_raw.permute(0, 2, 1).view(B, C, self.p_seq_len_y, -1).permute(0,2,1,3)
            score_y = self.mlstmnet(feature_y,m2)  # (B,k,C,N)
            ...
            return output, l_entropy, l_smooth
        else:
            # #back
            y = y[:,:,:self.v_num]
            mean_y = y.mean(dim=1, keepdim=True)
