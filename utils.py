...
class channel_AutoCorrelationLayer(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        if num == 2:
            ...
            queries = self.query_projection(queries).view(B,L, H, -1).permute(0,2,1,3) #B,H,L,d_keys
            keys = self.key_projection(keys).view(B,S, H, -1).permute(0,2,1,3) #B,H,S,d_keys
            values = self.value_projection(values).view(B,S, H, -1).permute(0,2,1,3) #B,H,S,d_keys
